...
public interface RetryRecordMapper {
    ...
    @Insert("insert into retry_record"
            ...
            + "values "
            + "(null,#{retryRecord.bizType},#{retryRecord.msgType},#{retryRecord.msgKey},#{retryRecord.extra},#{retryRecord.body},#{retryRecord.tryTimes},#{retryRecord.maxTryTimes},#{retryRecord.initErrorMessage},#{retryRecord.endErrorMessage},#{retryRecord.status},now(),now())")
    Integer create(@Param("retryRecord") RetryRecord retryRecord, @Param("metaConfig") IMetaConfig metaConfig);

    @Select("select * from retry_record "
    ...
    @Update("update retry_record set status=0,try_times=try_times+1,end_error_message='' where id=#{id}")
    Integer fail(@Param("id") Long id, @Param("metaConfig") IMetaConfig metaConfig);

    @Update("update retry_record set status=0, end_error_message=#{errorMessage} ,try_times=try_times+1 where id=#{id}")
    Integer failExp(@Param("id") Long id, @Param("errorMessage") String errorMessage, @Param("metaConfig") IMetaConfig metaConfig);

    @Update("update retry_record set status=1 where id=#{id}")
